    ...
    public void mapping() {
        ...
        final List<Employee> mappedEmployees =
                new MapHelper<>(employees)
                        .map(employee -> employee.withPerson(employee.getPerson().withFirstName("John"))) // change name to John .map(e -> e.withPerson(e.getPerson().withFirstName("John")))
                        .map(employee -> employee.withJobHistory(employee.getJobHistory().stream() // add 1 year to experience duration .map(e -> e.withJobHistory(addOneYear(e.getJobHistory())))
                                .map(jobHistoryEntry -> jobHistoryEntry.withDuration(jobHistoryEntry.getDuration() + 1))
                                .collect(Collectors.toList())))
                        .map(employee -> { // replace qa with QA
                                    List<JobHistoryEntry> newHistory = new ArrayList<>();
                                    for (int i = 0; i < employee.getJobHistory().size(); i++) {
                                        if (employee.getJobHistory().get(i).getPosition().equals("qa")) {
                                            newHistory.add(employee.getJobHistory().get(i).withPosition("QA"));
                                        } else {
                                            newHistory.add(employee.getJobHistory().get(i));
                                        }
                                    }
                                    return employee.withJobHistory(newHistory);
                                }
                        )
                        .getList();
    ...
    public void lazy_mapping() {
        ...
        final List<Employee> mappedEmployees =
                LazyMapHelper.from(employees)
                        .map(employee -> employee.withPerson(employee.getPerson().withFirstName("John"))) // change name to John
                        .map(employee -> employee.withJobHistory(employee.getJobHistory().stream() // add 1 year to experience duration .map(e -> e.withJobHistory(addOneYear(e.getJobHistory())))
                                .map(jobHistoryEntry -> jobHistoryEntry.withDuration(jobHistoryEntry.getDuration() + 1))
                                .collect(Collectors.toList())))
                        .map(employee -> { // replace qa with QA
