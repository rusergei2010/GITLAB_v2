...
//            throw new UnsupportedOperationException();
        ...
        final List<Employee> mappedEmployees =
                new MapHelper<>(employees)
                        ...
                        * */
                        ...
                        .map(e -> e.withJobHistory(QAUpperCase(e.getJobHistory())))
                        .getList();
    ...
    private List<JobHistoryEntry> QAUpperCase(List<JobHistoryEntry> jobHistory) {
        jobHistory.replaceAll(jobHistoryEntry ->
                jobHistoryEntry.getPosition().toUpperCase().equals("QA") ?
                        jobHistoryEntry.withPosition(jobHistoryEntry.getPosition().toUpperCase())
                        : jobHistoryEntry);
        return jobHistory;
    }

    private List<JobHistoryEntry> addOneYear(List<JobHistoryEntry> jobHistory) {
        jobHistory.replaceAll(
                jobHistoryEntry -> jobHistoryEntry.withDuration(1 + jobHistoryEntry.getDuration()));
        return jobHistory;
    }
    ...
    private static class LazyMapHelper<T, R> {
        ...
        public <R2> LazyMapHelper<T, R2> map(Function<R, R2> f) {
            list.forEach(t -> func.apply(t));
            Function<T, R2> funcRes = t -> f.apply(func.apply(t));
            return new LazyMapHelper<>(list, funcRes);
        }
    ...
    public void lazy_mapping() {
        ...
        final List<Employee> mappedEmployees =
                LazyMapHelper.from(employees)
                        .map(t -> t.withPerson(t.getPerson().withFirstName("John"))) // change name to John
                        .map(t -> t.withJobHistory(addOneYear(t.getJobHistory())))
                        .map(t -> t.withJobHistory(QAUpperCase(t.getJobHistory())))
                        .force();
