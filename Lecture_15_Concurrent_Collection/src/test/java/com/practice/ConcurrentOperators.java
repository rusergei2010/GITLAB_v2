...
public class ConcurrentOperators {
  ...
  /**
   * Size() operation is weak in the map and return right value just in single threaded apps (!= 1000) size() can be
   * violated for HashMap even if it is calculated after all modifications But it doesn't cause ConcurrentModification
   * Exception of iterator if it is not extracted Insertion is not atomic for HashMap if it is performed in different
   * threads Run several times the test
   */
  @Test
  public void testSizeWeakAffectAndPutNull_Cannot_Insert_A_NEW_VALUE() throws ExecutionException, InterruptedException {
    ...
    map.entrySet().stream().filter(e -> !e.getKey().toString().equals(String.valueOf(e.getValue())))
        .forEach((entry) -> {
  ...
  /**
   * Size() operation is weak even in ConcurrentHashMap during com.data modification (but in the end of modifications it
   * gives correct size) But in the end of all modifications it gives correct result putIfAbsent() - Atomic operation of
   * Insertion Run several times the test ThreadSafe Concurrent Case
   */
  @Test
  public void testSizeWeakAffectWithoutNullInInsertion() throws ExecutionException, InterruptedException {
    ...
    concurrentHashMap.entrySet().stream().filter(e -> !e.getKey().toString().equals(String.valueOf(e.getValue())))
        .forEach((entry) -> {
          System.out.println("" + entry.getKey() + ":" + entry.getValue());
        });
  ...
  public void testConcurrentOperationSuccessWithConcurrentHashMap() throws ExecutionException, InterruptedException {
    ...
    concurrentHashMap.entrySet().stream().filter(e -> !e.getKey().equals(Integer.valueOf(e.getValue())))
        .forEach((entry) -> {
          System.out.println("" + entry.getKey() + ":" + entry.getValue());
        });
  ...
  /**
   * Concurrent modification of HashMap without iteration through it doesn't cause any issues or exceptions Use of
   * CompletableFuture (Consider ForkJoinPool.commonPool().submit()) instead Fail-Fast example with HashSet - Iteration
   * is not allowed if new elements are submitted into the NonThread-Safe Collection
   */
...
//        ForkJoinPool.commonPool().submit(()->{});
    ...
    CompletableFuture.allOf(futureA, futureB)
        .get(); // blocking operator - wait till two completablefuture are finished and return result
