...
public class Mapping {
    ...
    private static class MapHelper<T> {
        ...
        public <R> MapHelper<R> map(Function<T, R> f) {
            return new MapHelper<>(list.stream().map(f).collect(Collectors.toList()));
        }
    ...
    private List<JobHistoryEntry> addYear(List<JobHistoryEntry> jobHistory) {
    ...
    private List<JobHistoryEntry> positionToUpperCase(List<JobHistoryEntry> jobHistory) {
        return jobHistory.stream().map(
                e -> e.getPosition().equals("qa") ? e.withPosition(e.getPosition().toUpperCase()) :
                        e.withPosition(e.getPosition())).collect(Collectors.toList());
    }
}
