...
public class Lambda2 {
    ...
    public void testLambda() throws InterruptedException {
        ...
        List<Person> males = list.stream().filter(person -> person.gender == Person.Gender.MALE).collect(Collectors.toList());
        ...
        males = list.stream().filter(person -> person.gender == Person.Gender.MALE).filter(person -> person.getAge() > 10).collect(Collectors.toList());
        ...
        List<Person> peopleWithSalary = list.stream().filter(person -> (person.gender == Person.Gender.MALE && person.getAge() > 10)
                || person.gender == Person.Gender.FEMALE).collect(Collectors.toList());
        ...
        int age =(int) peopleWithSalary.stream().mapToInt(person -> person.getAge()).sum();
        ...
        list.stream().map(person -> person.getName().concat(" ")).forEach(sb::append);
        /*for (Person p : list) {
